	...
	acceptPostCnt(0)
{
	...
	vecSockContexts.clear();
	InitializeSRWLock(&srwLockSC);
}
...
BOOL IOCPBase::SendData(RecvSendData *data)
{
	if (data == NULL)
		return 0;

	SocketContext *socketContext = GetContextFromID(data->connectID);
	if (socketContext == NULL)
	{
		delete data;
		return 0;
	}
	IOContext *ioContext = socketContext->AllocateIoContext();
	//ioContext->ioSocket = socketContext->connSocket;
	ioContext->numOfBytes = data->bytes;
	ioContext->sendDataID = data->sendDataID;
...
BOOL IOCPBase::InitializeListenSocket()
{
	...
	{
		IOContext *ioContext = listenSockContext->AllocateIoContext();
		if (false == PostAccept(listenSockContext, ioContext))
		{
			listenSockContext->RecycleIOContext(ioContext);
			return false;
		}
	}
	AddSocketContext(listenSockContext);
	return true;
}
...
BOOL IOCPBase::PostRecv(SocketContext * sockContext, IOContext *ioContext)
{
	...
	int nBytesRecv = WSARecv(ioContext->ioSocket, &ioContext->wsaBuf, 1, &ioContext->numOfBytes, &dwFlags, &ioContext->overLapped, NULL);
...
BOOL IOCPBase::PostSend(SocketContext * sockContext, IOContext *ioContext)
{
	...
	if (::WSASend(ioContext->ioSocket, &ioContext->wsaBuf, 1, &ioContext->numOfBytes, dwFlags, &ioContext->overLapped, NULL) != NO_ERROR)
	{
...
BOOL IOCPBase::DoAccpet(SocketContext * sockContext, IOContext * ioContext)
{
	...
	OnConnectionEstablished(newSockContext->connectID);

	// 5. 建立recv操作所需的ioContext，在新连接的socket上投递recv请求
	IOContext *newIoContext = newSockContext->AllocateIoContext();
	...
	AddSocketContext(newSockContext);
	return true;
}

BOOL IOCPBase::DoRecv(SocketContext * sockContext, IOContext * ioContext)
{
	RecvSendData *data = new RecvSendData(ioContext, sockContext->connectID);
	OnRecvCompleted(data);
	ioContext->Reset();
...
{
	OnSendCompleted(sockContext->connectID, ioContext->sendDataID);
	sockContext->RecycleIOContext(ioContext);
	return 0;
}

BOOL IOCPBase::DoClose(SocketContext * sockContext)
{
	RemoveSocketContext(sockContext);
	InterlockedDecrement(&connectCnt);
	RELEASE(sockContext);
	return true;
}

void IOCPBase::AddSocketContext(SocketContext * sockContext)
{
	AcquireSRWLockExclusive(&srwLockSC);
	vecSockContexts.push_back(sockContext);
	ReleaseSRWLockExclusive(&srwLockSC);
}

void IOCPBase::RemoveSocketContext(SocketContext * sockContext)
{
	AcquireSRWLockExclusive(&srwLockSC);
	if (vecSockContexts.size() > 0)
	{
		auto it = vecSockContexts.begin();
		for (; it != vecSockContexts.end(); it++)
		{
			if (*it == sockContext)
			{
				vecSockContexts.erase(it);
				break;
			}
		}
	}
...
SocketContext * IOCPBase::GetContextFromID(ULONG id)
{
	SocketContext *sockContext = NULL;
	AcquireSRWLockShared(&srwLockSC);
	if (vecSockContexts.size() > 0)
	{
		auto it = vecSockContexts.begin();
		for (; it != vecSockContexts.end(); it++)
	...
	ReleaseSRWLockShared(&srwLockSC);
	return sockContext;
}
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(iocp->stopEvent, 0))
	{
		...
		if (!bRet)
		{
			...
			if (WAIT_TIMEOUT == dwErr)
			{
				...
				{
					iocp->OnConnectionClosed(sockContext->connectID);
			...
			{
				iocp->OnConnectionError(sockContext->connectID, dwErr);
